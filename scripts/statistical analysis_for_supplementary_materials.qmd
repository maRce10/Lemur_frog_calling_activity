...
# Instructions {.unnumbered .unlisted}
...
write.csv(x = param_grid, file = file.path(path, "direct_and_total_effect_model_data_frame.csv"), row.names = FALSE)
...
These are the regression results for the direct and total effects for each predictor:
```{r, results='asis'}
...
saveRDS(combined_draws, file.path(path, "combined_draws_for_total_and_direct_effects_24h_previous_rain.RDS"))
```
...
saveRDS(combined_draws, file.path(path, "combined_draws_for_total_and_direct_effects_48h_previous_rain.RDS"))
