...
setwd("/Volumes/Grabaciones Backup Plus Drive/Grabaciones SM4 2019/Sukia/");root.dir=getwd() 
...
setwd("/Users/fabiolachirino/Documents/datos aleatorios/");sel.dir=getwd() #cambio
...
setwd(root.dir); rec.list=list.files(datos[-1], pattern=rec.format, recursive=T) #Este es el que funcioooona
...
#BETI:
#rec.table <- read.table("~/Dropbox/Estudiantes/Fabiola_Chirino/sukiadata.txt", header = T, sep = "\t")
...
month = gsub(pattern = "Data/SUKIA_20[0-9]{2}", x = rec.list, replacement = "\\1")
...
day = gsub(pattern = "Data/SUKIA_20[0-9]{4}", x = rec.list, replacement = "\\1")
...
date = gsub(pattern = "Data/SUKIA_", x = rec.list, replacement = "\\1")
...
#CHANGE the code & RUN
d.strata=c(20190801,20190901,20191001,20191101,20191201,20200101,20200201, 20200301) 
...
for(i in 1:len.d.strata){
  for (k in 1:len.w.strata){
    ...
    if(length(sel.rec)>0){
      ...
      file.copy(paste(paste0(root.dir,"/SUKIA_", rec.table$date[sel.rec]),
                      rec.table$file.name[sel.rec], sep="/"),
                new.folder)   
    }
